...
const NAV_LINKS = [
  ...
  { href: '#vai-tro',     label: 'Vai Trò Đảng' },
      ...
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 66 }}>
        ...
        <nav style={{ display: 'flex', gap: '1.6rem' }} className="desktop-nav">
          {NAV_LINKS.map(({ href, label }) => (
            <a key={href} href={href} className="nav-link" style={navLinkStyle}>
              {label}
            </a>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            style={{ overflow: 'hidden', background: 'rgba(10,2,2,0.97)', borderTop: '1px solid rgba(212,175,55,0.15)' }}
          >
            <div style={{ display: 'flex', flexDirection: 'column', padding: '1rem 1.5rem 1.5rem' }}>
              {NAV_LINKS.map(({ href, label }) => (
                <a key={href} href={href} onClick={() => setMenuOpen(false)} className="nav-link" style={{ ...navLinkStyle, padding: '0.75rem 0', borderBottom: '1px solid rgba(212,175,55,0.1)' }}>
                  {label}
                </a>
              ))}
            </div>
